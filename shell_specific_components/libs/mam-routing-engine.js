/*
  * Module Owner : Eshwaran Veerabahu (git: eash-veera)
  * Module : Mambati Routing Engine
  * Companion Modules : Sticky States, Future States, Deep State Redirect, Transiton promise
  * Build type : Monolithic build (all modules self injected)
  @version 0.2.6
  @licence MIT Licence, http://www.opensource.org/licenses/MIT 
*/
...
(function (root, factory) {
    ...
    var mod_core = angular.module("mam.routing.engine.core", [ "ui.router" ]);
...
angular.module('mam.routing.engine.dsr', [ 'mam.routing.engine.core' ]).config([ "$provide", function ($provide) {
...
angular.module('mam.routing.engine.dsr').service("$deepStateRedirect", [ '$rootScope', '$state', '$injector', function ($rootScope, $state, $injector) {
  var lastSubstate = {};
...
angular.module('mam.routing.engine.dsr').run(['$deepStateRedirect', function ($deepStateRedirect) {
  // Make sure $deepStateRedirect is instantiated
}]);

angular.module("mam.routing.engine.sticky", [ 'mam.routing.engine.core' ]);

var mod_sticky = angular.module("mam.routing.engine.sticky");
...
angular.module("mam.routing.engine.sticky").run(["$stickyState", function ($stickyState) {
  _StickyState = $stickyState;
}]);

angular.module("mam.routing.engine.sticky").config(
...
(function(angular, undefined) {
  var app = angular.module('mam.routing.engine.future', [ 'mam.routing.engine.core' ]);
...
angular.module('mam.routing.engine.previous', [ 'mam.routing.engine.core', 'mam.routing.engine.transition' ]).service("$previousState",
  [ '$rootScope', '$state', '$q',
...
angular.module('mam.routing.engine.previous').run(['$previousState', function ($previousState) {
  // Inject $previousState so it can register $rootScope events
}]);


angular.module("mam.routing.engine.transition", [ 'mam.routing.engine.core' ]).config( [ "$provide",  function ($provide) {
...
(function () {
  "use strict";
  var app = angular.module("mam.routing.engine.statevis", [ 'mam.routing.engine.core', 'mam.routing.engine.sticky'  ]);
...
angular.module("mam.routing.engine",
  [
    'mam.routing.engine.core',
    'mam.routing.engine.dsr',
    'mam.routing.engine.future',
    'mam.routing.engine.previous',
    'mam.routing.engine.statevis',
    'mam.routing.engine.sticky',
    'mam.routing.engine.transition'
  ]);
