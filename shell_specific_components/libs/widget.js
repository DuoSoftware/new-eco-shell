angular.module('widget', []).controller('clockWidgetController', ['$scope', '$mdDialog', function ($scope, $mdDialog) {
...
//        };
    ...
    .controller('weatherWidgetController', ['$scope', '$http', '$mdDialog', function ($scope, $http, $mdDialog) {
        $scope.loadWeather = function (data) {
            ...
            //complete config  
            //            function () {
            $http.get('http://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20weather.forecast%20where%20woeid%20in%20(select%20woeid%20from%20geo.places(1)%20where%20text%3D%22' + data + '%22)&format=json&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys').success(function (data) {
                console.log(data);
        ...
        (function () {
            ...
            $scope.getProfilePicture = function () {
                ...
                client.onGetOne(function (data) {
                    if (data) $scope.profilePicture = data.Body;
                    console.log(data);
                });
                client.onError(function (data) {
                    console.log(data);
    ...
    , [28, 48, 40, 19, 86, 27, 90]
    ...
    }).directive('clockComponent', function ($mdDialog) {
        return {
            //        restrict: 'E',
            //controller: 'clockWidgetController',
            template: " <div class='widget-card' style='padding:4px;' layout='column' layout-align='start center'><ul id='clockclock'><li id='clockhour'></li><li id='clockmin'></li><li id='clocksec'></li></ul></div>",
        ...
        var clockEle = element[0].firstElementChild.children[0].children;
//console.log(clockEle);
        ...
        $(function () {
            setInterval(function () {
                ...
                var sdegree = seconds * 6;
                var srotate = "rotate(" + sdegree + "deg)";
                clockEle[2].style.transform=srotate;
            }, 1000);
            setInterval(function () {
                var hours = new Date().getHours();
                //clockEle[0].innerHTML=hours;
                var mins = new Date().getMinutes();
                //clockEle[1].innerHTML=mins;
                var hdegree = hours * 30 + (mins / 2);
                var hrotate = "rotate(" + hdegree + "deg)";
                clockEle[0].style.transform=hrotate;
            }, 1000);
            setInterval(function () {
                var mins = new Date().getMinutes();
                //$scope.clockComponentMins = mins;
                ...
                var mdegree = mins * 6;
                var mrotate = "rotate(" + mdegree + "deg)";
                clockEle[1].style.transform=mrotate;
            }, 1000);
        });
    ...
    <div  class="weather widget-card " style="width: 100%;height: 80%;">\
 <md-button style="min-width: 10px;" ng-click="weatherComponentSelectCity($event);"><md-icon md-svg-icon="icons/ic_menu_24px.svg" alt="menu"></md-button>\
...
<div class="weather-forcast-container" layout="row">\
            <div ng-repeat="f in weatherForcast | limitTo:5" class="weather-forcast-item" flex layout="row" layout-align="center center" ><p>{{f.day}}<br><span>{{f.high}} F</span></p></div>\
            </div> </div>'
        };
