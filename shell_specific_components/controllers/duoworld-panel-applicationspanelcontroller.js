...
(function () {
    ...
    var applicationsPanelCtrl = function ($rootScope, $scope, shellDataFactory) {

        //variable declaration
        var applications = this;
        applications.shellConfig = shellDataFactory.getShellConfig();
        applications.allApps = shellDataFactory.getAllApps();
        //************************************************************************************************
    ...
    applicationsPanelCtrl.$inject = ['$rootScope', '$scope', 'shellDataFactory'];
    angular.module('mambatiFrameworkShell').controller('applicationsPanelCtrl', applicationsPanelCtrl);
})();
...
$scope.retrivedAppPanelsFormatedDataPush = function () {
    for (var i = 0; i < $scope.frameworkpannelApps.length; i++) {
        ...
        $scope.frameworkpannelAppCollection.push({
            applicationID: appTempRef.ApplicationID,
            applicationTitle: appTempRef.Name,
            applicationCategory: appTempRef.Description,
