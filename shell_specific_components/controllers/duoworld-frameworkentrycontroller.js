...
(function () {
    ...
    var duoWorldFrameworkEntryCtrl = function ($scope, $state, $mdDialog, duoWorldTennantInfoFactory, $http, $presence, $auth, $apps, $helpers) {
        ...
        $scope.submitRegistrationDetails = function (ev) {
            var registrationSubmissionDetails = JSON.stringify({
                "EmailAddress": $scope.registerDetails.email,
                "Name": $scope.registerDetails.fullName,
                "Password": $scope.registerDetails.password,
                "ConfirmPassword": $scope.registerDetails.confirmPassword
            });
    ...
    angular.module('mambatiFrameworkShell').controller('duoWorld-Framework-Entry-Ctrl', duoWorldFrameworkEntryCtrl);
